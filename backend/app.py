...
@app.route('/train_test_data', methods=['GET'])
def get_train_test_data():
    # Combine train and test sets into a single DataFrame for demonstration
    x_train, x_test = collab_filtering.trainset, collab_filtering.testset
    combined_df = pd.concat([x_train, x_test], keys=['train', 'test'])
    csv_data = combined_df.to_csv(index=False)
    response = make_response(csv_data)
    response.headers['Content-Type'] = 'text/csv'
    response.headers['Content-Disposition'] = 'attachment; filename=train_test_data.csv'
    return response
